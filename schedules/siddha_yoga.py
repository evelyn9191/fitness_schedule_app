...
from schedules.isportsystem import ISportSystemSchedulesHandler, parse_new_isportsystem_html, SIDDHA_YOGA_FUGNEROVA_VENUE_COLOR
...
def get_schedule():
    ...
    session.get("https://siddha-yoga.isportsystem.cz/")
    
    for date in dates_to_parse_from:
        handler = ISportSystemSchedulesHandler(GYM, "siddha-yoga", date)
        # Use POST with correct parameters (same as Yoga Karlin)
        post_data = {
            "id_sport": 5,
            "day": date.day,
            "month": date.month,
        ...
        # Use shared parser with Fugnerova venue filter
        parsed_schedules.extend(parse_new_isportsystem_html(response.text, GYM, SIDDHA_YOGA_FUGNEROVA_VENUE_COLOR))
